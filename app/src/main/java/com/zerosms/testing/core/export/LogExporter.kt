package com.zerosms.testing.core.export
...
import com.zerosms.testing.core.root.RootActivity
import com.zerosms.testing.core.root.RootActivityType
import com.zerosms.testing.core.model.TestResult
import com.zerosms.testing.core.model.TestStatus
import java.io.File
...
object LogExporter {
    ...
    fun exportActivityLogs(context: Context, logs: List<RootActivity>? = null): Boolean {
        return try {
            ...
            val fileName = "007smsdev_logs_$timestamp.txt"
            
            val logFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS", Locale.US)
            
            val content = buildString {
                appendLine("========================================")
                appendLine("007SMS DEV Activity Log Export")
    ...
    fun exportTestResults(context: Context, results: List<TestResult>): Boolean {
        return try {
            ...
            val dateFormat = SimpleDateFormat("yyyy-MM-dd_HH-mm-ss", Locale.US)
            val timestamp = dateFormat.format(Date())
            val fileName = "007smsdev_results_$timestamp.csv"
    ...
    fun exportTestSummary(context: Context, results: List<TestResult>): Boolean {
        return try {
            val dateFormat = SimpleDateFormat("yyyy-MM-dd_HH-mm-ss", Locale.US)
            val timestamp = dateFormat.format(Date())
            val fileName = "007smsdev_summary_$timestamp.txt"
            ...
            val content = buildString {
                appendLine("========================================")
                appendLine("007SMS DEV Test Results Summary")
    ...
    private fun shareTextFile(context: Context, fileName: String, content: String, mimeType: String) {
        ...
        val shareIntent = Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "007SMS DEV Export: $fileName")
