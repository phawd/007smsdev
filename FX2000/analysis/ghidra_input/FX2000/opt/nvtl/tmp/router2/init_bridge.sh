...
if [ -d /sys/devices/virtual/net/bridge0/brif/ ]; then 
    print_if_failed ls /sys/devices/virtual/net/bridge0/brif/ >/opt/nvtl/tmp/router2/br_ifc
...
if [ $ifc == "wlan1" ] ; then 
	print_if_failed brctl addif bridge0 wlan1
fi 
...
if [[ !  -z  $mac  ]] ; then 
   print_if_failed ifconfig bridge0 hw ether $mac 
fi 
