...
- **Notes:** The checked-in `local.properties` points to a Windows host path, so Linux and WSL builds require overriding it with a valid SDK path before Gradle tasks can run. Download Android Studio/SDK: https://developer.android.com/studio
...
- **Notes:** Install `pyserial` (e.g., `pip install pyserial`) to enable the tool. Download: https://pypi.org/project/pyserial/
...
- **Notes:** The helper assumes Android Platform Tools are installed and available on the PATH. Download: https://developer.android.com/studio/releases/platform-tools
